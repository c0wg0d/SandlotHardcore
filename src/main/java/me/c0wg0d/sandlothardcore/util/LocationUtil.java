...
import me.c0wg0d.sandlothardcore.async.Callback;
...
public enum LocationUtil {
            ...
            for (int dx = 1; dx <= 30; dx++) {
                for (int dz = 1; dz <= 30; dz++) {
                    ...
                    if (isSafeLocation(spawnLocation)) {
                        ...
                        Bukkit.getLogger().fine("found safe location " + spawnLocation + " near " + loc + ", looking at " + lookAt);
    ...
    private static boolean isLiquidOrAir(Material m) {
        return m.equals(Material.AIR) || m.equals(Material.CAVE_AIR) || m.equals(Material.WATER) || m.equals(Material.LAVA);
    }
